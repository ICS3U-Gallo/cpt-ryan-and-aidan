...
class Range_Enemy_cross(Range_Enemy):
    ...
    def fire(self, game):
        ...
        for angle in range(0, 360, 90):
            bullet = arcade.Sprite("images/rock.png",
                                   sprite_scale/math.sqrt(2))
...
class Range_Enemy_Aim(Range_Enemy):
    ...
    def fire(self, game):
        # Fire at player
        bullet = arcade.Sprite("images/rock.png", sprite_scale)
        bullet.center_x = self.center_x
        bullet.center_y = self.center_y
...
class Range_Enemy_Aim_2(Range_Enemy):
    ...
    def fire(self, game):
        ...
        for ang in range(int(math.degrees(self.ang))-30,
                         int(math.degrees(self.ang))+31, 30):
            bullet = arcade.Sprite("images/rock.png", sprite_scale)
            bullet.center_x = self.center_x
            bullet.center_y = self.center_y
            bullet.angle = ang
...
class Boss(arcade.Sprite):
    ...
    def fire_alldir(self, game):
        ...
        for angle in range(0, 360, 45):
            bullet = arcade.Sprite("images/fireball.png",
                                   sprite_scale/math.sqrt(2))
            bullet.center_x = self.center_x
            bullet.center_y = self.center_y
            bullet.angle = angle + 90
...
def bullet_hit(bullet, game):
    ...
    hit_wall = arcade.check_for_collision_with_list(bullet, game.rooms[
        game.current_room].wall_list)
    if bullet.top < 0:
        bullet.kill()
    elif len(hit_wall) > 0:
        explosion = Explosion(game.explosion_texture_list)
...
def bullet_removal(game):
    ...
    if (game.player_sprite.center_y < 0 or game.player_sprite.center_y >
       screen_height or game.player_sprite.center_x < 0 or
       game.player_sprite.center_x > screen_width):
